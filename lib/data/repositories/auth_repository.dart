...
class AuthRepository extends AuthInterface {
  ...
  Future<bool> updateCookies() async {
    inapp.CookieManager cookieManager = inapp.CookieManager.instance();
    var cookie = await cookieManager.getCookies(url:  kBaseUrl.webUri);
    var c = "";
    if (cookie.isEmpty) return false;
    for (var element in cookie) {
      c += '${element.name}=${element.value}; ';
    }
    cookie = await cookieManager.getCookies(url:  "https://signin.intra.42.fr/users/sign_in".webUri);
    try{
  ...
  Future<Pair<bool, String>> checkApiAccess(String endpoint, String redirectUrl, String clientId, String clientSecret) async{
    ...
    try{
      ...
      return Pair(false, _.response?.headers["www-authenticate"]?.toString() ?? _.message ?? "Something went wrong");
    }
